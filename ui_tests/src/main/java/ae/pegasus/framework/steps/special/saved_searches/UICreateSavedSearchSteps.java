...
public class UICreateSavedSearchSteps {
    @Then("I should see Create Saved Search page")
    public void iShouldSeePage() {
        Asserter.getAsserter().softAssertTrue(
                Pages.createSavedSearchPage().isPageDisplayed(),
                "Create Saved Search page is displayed",
                "Create Saved Search page is NOT displayed"
        );
    }

    private void checkSearchingCriteria(SearchingCriteriaField field, String...expectedValue) {
        AssertUtils.checkArrays(
                Pages.createSavedSearchPage().getSearchingCriteria(field),
                expectedValue,
                "Searching Criteria '" + field.getFieldName() +"' on the Create Saved Search page");
    }

    @Then("I should see Query ($searchQuery) as Searching Criteria on the Create Saved Search page")
    ...
    @Then("I should see Record Type ($recordType) as Searching Criteria on the Create Saved Search page")
    ...
    @Given("I set Name to ($name) on the Create Saved Search page")
    public void iSetName(String name) {
        Pages.createSavedSearchPage().setPropertyValue(NAME, name);
    }

    @Given("I set Description to ($description) on the Create Saved Search page")
    public void iSetDescription(ExamplesTable description) {
        Pages.createSavedSearchPage().setPropertyMultiValues(
                DESCRIPTION,
                ParametersHelper.processExampleTable(description)
        );
    }

    @Given("I set Classification to ($classification) on the Create Saved Search page")
    public void iSetClassification(String classification) {
        Pages.createSavedSearchPage().setPropertyValue(CLASSIFICATION, classification);
    }

    @Given("I set Organization Unit to ($organizationUnit) on the Create Saved Search page")
    ...
    @Given("I set Files to ($files) on the Create Saved Search page")
    ...
    @Given("I setup Scheduled Search to start from ($fromDate) every ($repeatInterval) till ($endDate) on the Create Saved Search page")
    public void iSetupScheduledSearch(String fromDate, String repeatInterval, String endDate){
        Pages.createSavedSearchPage().setupSchedule(
    ...
    public void iSaveSearch() {
        Pages.createSavedSearchPage().clickButton(SAVE_SEARCH);
    }
}
